...
export default function AuditorDashboard() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        const [usersRes, transactionsRes, hydrogenRes] = await Promise.all([
          API.get("/api/users"),
          API.get("/api/cointransactions"),
          API.get("/api/hydrogenrequests")
        ]);

        setUsers(usersRes.data || []);
        setTransactions(transactionsRes.data || []);
        if (hydrogenRes.data && hydrogenRes.data.length > 0) {
          setHydrogenRequests(hydrogenRes.data);
        } else {
  ...
  // Filtered transactions and requests
  const filteredTransactions = filterType ? transactions.filter(t => t.type === filterType) : transactions;
  const filteredHydrogen = filterStatus ? hydrogenRequests.filter(r => r.status === filterStatus) : hydrogenRequests;

  // Analytics data for graphs
  const coinData = {
    labels: filteredTransactions.map(tx => new Date(tx.createdAt).toLocaleDateString()),
    datasets: [
      {
        label: "Coins",
        data: filteredTransactions.map(tx => tx.coins),
        borderColor: "#10b981",
  ...
  const hydrogenData = {
    labels: filteredHydrogen.map(r => new Date(r.createdAt).toLocaleDateString()),
    datasets: [
      {
        label: "Hydrogen (kg)",
        data: filteredHydrogen.map(r => r.amount),
        borderColor: "#3b82f6",
    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      <AnimatedCard className="mb-8">
        <div className="flex items-center gap-4 mb-4">
          ...
          <h3 className="text-2xl font-bold">Welcome, Auditor!</h3>
        </div>
            ...
            <tbody>
              {demoUsers.filter(u => u.name.toLowerCase().includes(search.toLowerCase()) || u.email.toLowerCase().includes(search.toLowerCase())).map((u) => (
                <tr key={u._id} className="hover:bg-slate-800 transition">
                  <td>{u.name}</td>
                  <td>{u.email}</td>
                  <td><span className={`px-2 py-1 rounded-full text-xs ${u.role === "auditor" ? "bg-blue-500 text-white" : "bg-emerald-500 text-white"}`}>{u.role}</span></td>
                  <td>{u.companyName}</td>
